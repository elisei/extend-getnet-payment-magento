...
namespace O2TI\ExtendGetnetPaymentMagento\Plugin\Gateway\Http\Client;
...
class PreAuthAcceptPaymentClient implements ClientInterface
{
    /**
    ...
    public function placeRequest(TransferInterface $transferObject)
    {
        ...
        $requestToAdjustment = array_merge($request, ['currency' => 'BRL']);
        $client->setUri($url.'/v1/payments/credit/'.$paymentId.'/adjustment');
        $client->setConfig(['maxredirects' => 0, 'timeout' => 45000]);
        $client->setHeaders('Authorization', 'Bearer '.$apiBearer);
        $client->setRawData($this->json->serialize($requestToAdjustment), 'application/json');
        $client->setMethod(ZendClient::POST);
        $responseBodyAjustment = $client->request()->getBody();
        $this->logger->debug(
            [
                'url'      => $url.'/v1/payments/credit/'.$paymentId.'/adjustment',
                'request'  => $this->json->serialize($requestToAdjustment),
                'response' => $this->json->serialize($responseBodyAjustment),
            ]
        );
